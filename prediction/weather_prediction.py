...
def result():
  ...
  csv_path = os.path.join(os.path.dirname(__file__), "HCM_Dataset.csv")
  ...
  client.publish('v1/devices/me/telemetry', json.dumps({"humi_input": Humidity, "temp_input": Temperature}), 1)
  ...
  res_str = ''
  if prediction[0]==0:
    client.publish('v1/devices/me/telemetry', json.dumps({"weather": "CLEAR"}), 1)
    res_str = 'CLEAR'
  elif prediction[0]==1:
    client.publish('v1/devices/me/telemetry', json.dumps({"weather": "CLOUDY"}), 1)
    res_str = 'CLOUDY'
  elif prediction[0]==2:
    client.publish('v1/devices/me/telemetry', json.dumps({"weather": "RAINY"}), 1)
    res_str = 'RAINY'
  else:
    client.publish('v1/devices/me/telemetry', json.dumps({"weather": "SUNNY"}), 1)
    res_str = 'SUNNY'
